      ...
      {submissions.length === 0 ? (
        ...
        <div className="rounded-xl border border-primary/10 bg-card/80 p-5 shadow-sm backdrop-blur-sm">
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {submissions.map((s, i) => (
                <tr key={s.id} className={`border-b border-border border-l-2 border-l-transparent transition-colors hover:border-l-primary hover:bg-primary/5 ${i % 2 === 1 ? 'bg-muted/40' : ''}`}>
